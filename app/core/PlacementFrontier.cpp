...
void erasePlotRef(std::vector<FrontierRef>& bucket, const FrontierRef& ref) {
    bucket.erase(std::remove_if(bucket.begin(), bucket.end(),
                                [&](const FrontierRef& entry) {
                                    return entry.roadId == ref.roadId
                                           && entry.bankIndex == ref.bankIndex
