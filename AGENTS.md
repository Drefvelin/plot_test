...
1. `TownBuilder::build(config)` — jc_voronoi → roads; corridors; **`sanitizeRoadGraphAtWater`** (once, all non-bridge primaries); collect **`watersideJunctionIds`** (raster probe: forbidden terrain within `bridge_waterside_max_dist` of junction on terrain image); **`mergeWatersideJunctions`** (cluster nearby waterside junctions within `shore_junction_merge_dist`, default 2 u); **`resolveBridges`** (pair waterside junctions only, shortest-first greedy with pending-partner deferral, snap, create); **`buildBridgeBuckets`** (per-bridge road buckets within `bridge_bucket_hops` of each end, default 2; seed reveal nearest center); `assignRoadSideInwards(terrain)`; `ensureTownFrontageInitialized` (segments + frontier once)
...
- **Auto-grow** — CLI `--auto-grow [N]` raises target by **one building per step** (`--auto-grow-ms` interval from config, default 50 ms). **`--auto-exit` alone** quits right after town load (writes logs + memory report). With `--auto-grow`, `--auto-exit` waits until the target is placed (exit code = failure count). Config `growth.auto_exit: true` matches CLI. Bare `--auto-grow` defaults to **200** (or `town.yml` total if higher). **G** toggles in-app auto-grow to max.
...
- `Hud` — screen-fixed bar at top; drag to set 0…max buildings; shows `{placed} placed / {target} target` and **Failures: N** under the slider (red when N > 0, green when 0). **T** cycles terrain overlay modes; **Z** zone hop tint; **B** / **Bridges** toggle bridge debug view (terrain image + **white probe circle** per junction at `bridge_waterside_max_dist`, **red X** at raster water hit, purple=waterside / red=miss junction discs, labels `J{id} R{roadIds}` + brown bridges `B{id}`); **P** / **Biome plots** shows terrain-tagged building plot outlines plus rotated labels (`type`, `placement/prefer` e.g. `farm` / `prox/plains`); **G** toggles auto-grow.
