...
1. `TownBuilder::build(config)` — jc_voronoi → roads; corridors; **`sanitizeRoadGraphAtWater`** (once, all non-bridge primaries); **`mergeWatersideJunctions`** (cluster nearby shore junctions within `shore_junction_merge_dist`, default 2 u); **`resolveBridges`** (outline shore junctions, shortest-first greedy with pending-partner deferral, snap, create); `assignRoadSideInwards(terrain)`; `ensureTownFrontageInitialized` (segments + frontier once)
